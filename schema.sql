...
--
--  O dono/administrador (super-user) está definido como:
--      ricardo.moura@gmail.com
--  (garanta que existe um utilizador com este email em Authentication → Users
--   ANTES de correr; senão o admin não é criado.)
-- =====================================================================
...
-- Definir o DONO/ADMIN inicial (super-user)
insert into public.app_admins (user_id)
select id from auth.users where lower(email) = lower('ricardo.moura@gmail.com')
...
-- ---------------------------------------------------------------------
-- 6) COPIAR MENUS PARA OS NOVOS BARES
--    LADO Porto = cópia do LADO Régua · Ceira Wines = cópia do Bagaúste
--    (copia os dados ATUAIS; as imagens ficam a usar o logótipo de cada bar)
-- ---------------------------------------------------------------------
-- LADO Porto = cópia do LADO Régua
delete from public.products where bar_id = (select id from public.bars where slug='lado-porto');
delete from public.categories where bar_id = (select id from public.bars where slug='lado-porto');
do $$
declare src uuid; dst uuid; c record; newcat uuid;
begin
  src := (select id from public.bars where slug='lado-regua');
  dst := (select id from public.bars where slug='lado-porto');
  for c in select * from public.categories where bar_id = src order by sort_order, created_at loop
    insert into public.categories(bar_id, name_pt, name_en, sort_order, is_active)
      values (dst, c.name_pt, c.name_en, c.sort_order, c.is_active) returning id into newcat;
    insert into public.products(bar_id, category_id, name_pt, name_en, description_pt, description_en, price, image_url, sort_order, is_active)
      select dst, newcat, name_pt, name_en, description_pt, description_en, price, null, sort_order, is_active
      from public.products where category_id = c.id order by sort_order;
  end loop;
end $$;

-- Ceira Wines = cópia do Bagaúste
delete from public.products where bar_id = (select id from public.bars where slug='ceira-wines-guiaes');
delete from public.categories where bar_id = (select id from public.bars where slug='ceira-wines-guiaes');
do $$
declare src uuid; dst uuid; c record; newcat uuid;
begin
  src := (select id from public.bars where slug='bagauste');
  dst := (select id from public.bars where slug='ceira-wines-guiaes');
  for c in select * from public.categories where bar_id = src order by sort_order, created_at loop
    insert into public.categories(bar_id, name_pt, name_en, sort_order, is_active)
      values (dst, c.name_pt, c.name_en, c.sort_order, c.is_active) returning id into newcat;
    insert into public.products(bar_id, category_id, name_pt, name_en, description_pt, description_en, price, image_url, sort_order, is_active)
